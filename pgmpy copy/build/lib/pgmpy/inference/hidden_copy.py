...
class Mplp(Inference):
    ...
    def _update_message(self, sending_cluster):
        ...
        for current_intersect in sending_cluster.intersection_sets_for_cluster_c:
            ...
            #print('theta[current_intersect]: ', theta[current_intersect])
            #print('self.sum_messages[current_intersect]: ', self.sum_messages[current_intersect])
    ...
    def _update_message_triplet_edge(self, sending_cluster):
        ...
        print('sending_cluster: ', sending_cluster)
        print('intersection_sets_for_cluster_c: ', sending_cluster.intersection_sets_for_cluster_c)
        ...
        for current_intersect in sending_cluster.intersection_sets_for_cluster_c:
            ...
            print('LENGTH: ', len(sending_cluster.intersection_sets_for_cluster_c))
            phi_triplet_edge *= 1/len(sending_cluster.intersection_sets_for_cluster_c)
            # Step. 4) Subtract \delta_i^{-f}
            updated_results.append(
    ...
    def _local_decode(self):
        ...
        #print('objective_new for local decode')
        #for node in self.objective_new:
        #    if len(node) == 1: 
        ...
        #     print('values: ', self.factors[variable][0].values)
        #     print('node decode: ', 
        ...
        #             ]
        #         print('index: ', index)
        #         edge_value = cluster.cluster_potential.reduce(
        #                 index, inplace=False
        #             ).values
    ...
    def check_feasibility(self):
        ...
        for factor in self.model.get_factors():
            scope = factor.scope()
            #print('scope in check feasibility: ', scope)
            if len(scope) >= 2:
                ...
                for var in scope:
                    ...
                    #if min(message.values) < 0:
                    #    print(message)
                    #    raise ValueError("POSITIVE MESSAGE VALUES EXPECTED")
                    theta_constraint += message
                    dual_dict[frozenset(var)] += message
                    message_list.append(message)
                #if np.amax(theta_constraint.values) != np.amax(factor.values):
                    #print('message_list: ', message_list[0])
                    #print('message_list: ', message_list[1])
                    #print('factor: ', factor)
                    #raise ValueError('THETA CONSTRAINT VIOLATION') 
        for factor in self.model.get_factors():
            scope = factor.scope()
            if len(scope) == 1:
    ...
    def _is_converged(self, dual_threshold=None, integrality_gap_threshold=None):
        ...
        #    sending_cluster = self.cluster_set[frozenset(scope)]
        #    for var in scope:
        #        sending_cluster.message_from_cluster[frozenset(var)]
        #self.check_feasibility()
    ...
    def _update_triangles(self, triangles_list):
        ...
        for triangle_vars in triangles_list:
            ...
            if len(triplet_intersections) < 2:
                print('triplet_intersections: ', triplet_intersections)
                raise ValueError('triplet intersections too short')
            ...
            for intersect in triplet_intersections:
                if not self.active_messages[frozenset(intersect)]:
                    ...
                    print('active edge to edge: ', intersect)
    ...
    def _get_triplet_scores(self, triangles_list):
        ...
        for triplet in triangles_list:
            ...
            #print('triplet in scores: ', triplet)
            triplet_scores[frozenset(triplet)] = score

        #print('sanity triplet scores: ', triplet_scores)
            ...
            for triplet_number in range(len(sorted_scores)):
                ...
                for pair in it.combinations(addition,2):
                    if frozenset(pair) in edges:
                        size += 1
                if size < 2:
                    raise ValueError('tighten triplet less than two intersects')
